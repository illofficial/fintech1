...
GET_FINTECH_TRANSACTIONS_TOOL: ChatCompletionToolParam = {
    ...
    "function": {
        ...
        "description": "Fetch transaction records from the fintech database for a given account and date range.",
...
async def default_fintech_db_lookup(query: FintechTransactionQuery) -> list[TransactionRecord]:
    ...
    sample_transactions = [
        TransactionRecord(id="TXN-1001", account_id=query.account_id, date=date(2026, 7, 1), amount=-45.20, currency="USD", category="payment", merchant="Cloud Services Inc."),
        TransactionRecord(id="TXN-1002", account_id=query.account_id, date=date(2026, 7, 5), amount=2500.00, currency="USD", category="transfer", merchant="Payroll Deposit"),
        TransactionRecord(id="TXN-1003", account_id=query.account_id, date=date(2026, 7, 10), amount=-120.00, currency="USD", category="withdrawal", merchant="ATM Withdrawal"),
    ]
    filtered = [
        row for row in sample_transactions
...
class AgentOrchestrator:
    ...
    async def _execute_tool(self, tool_name: str, raw_arguments: str) -> str:
        ...
        try:
            ...
            if len(records) > MAX_TRANSACTIONS_PER_RESPONSE:
                logger.warning("Truncating tool response from %d to %d items", len(records), MAX_TRANSACTIONS_PER_RESPONSE)
                records = records[:MAX_TRANSACTIONS_PER_RESPONSE]
                
            return json.dumps({"transactions": [r.model_dump(mode="json") for r in records]})
        except Exception as exc:
            logger.exception("Graceful degradation: Tool handler '%s' failed", tool_name)
            return json.dumps({"error": "Fintech database is temporarily unavailable. Prompt user accordingly."})

    async def _apply_tool_calls(
        self, messages: list[ChatCompletionMessageParam], assistant_message: ChatCompletionMessage
    ) -> None:
        messages.append(self._assistant_param(assistant_message))
        for tool_call in (assistant_message.tool_calls or []):
            tool_result = await self._execute_tool(tool_call.function.name, tool_call.function.arguments)
    ...
    async def _run_tool_loop(self, messages: list[ChatCompletionMessageParam]) -> tuple[list[ChatCompletionMessageParam], ChatCompletion]:
        """Unified execution loop ensuring DRY compliance across public APIs."""
        logger.info("Starting agent execution loop (max_iterations=%d)", self._max_iterations)
        
        for iteration in range(self._max_iterations):
            logger.debug("Running loop iteration %d/%d", iteration + 1, self._max_iterations)
            try:
            ...
            if not assistant_message.tool_calls:
                if assistant_message.content is None:
                    logger.warning("Anomaly: OpenAI returned empty content and no tool calls at iteration %d", iteration)
                return messages, completion

            await self._apply_tool_calls(messages, assistant_message)

        logger.error("Agent execution loop exceeded max_iterations=%d", self._max_iterations)
        raise MaxIterationsExceededError(self._max_iterations)

    async def build_context(self, message: str, system_prompt: str = DEFAULT_SYSTEM_PROMPT) -> list[ChatCompletionMessageParam]:
        """Resolve tools and return message history ready for streaming downstream."""
        messages: list[ChatCompletionMessageParam] = [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": message},
        ]
        context, _ = await self._run_tool_loop(messages)
        return context
    ...
    async def run(self, request: AgentRequest | str) -> AgentResponse:
        ...
        _, completion = await self._run_tool_loop(messages)
        choice = completion.choices[0]
        
        return AgentResponse(
            content=choice.message.content or "",
            model=completion.model,
            iterations=self._max_iterations,  # Upper bound fallback info
